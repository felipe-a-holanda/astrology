...
class Planet(object):
    ...
    PLANET_NAMES = [i.lower() for i in PLANET_NAMES]
    PLANET_GLYPHS = u'☉☽☿♀♂♃♄♅♆♇'
    ...
    def draw(self, dwg, center, radius):
        ...
        svg = dwg.svg(id=self.name, class_='svg_obj svg_planet', visibility="hidden")
...
class Aspect(object):
    ...
    def draw(self, dwg, center, radius):
        ...
        svg = dwg.svg(id = self.name, class_='svg_obj svg_aspect')
        
        highlight = dwg.line(p1,p2,id = self.name+'-highlight', visibility='hidden')
        
        svg.add(highlight)
        #svg = dwg.svg(id = self.name)
        line = dwg.line(p1,p2, id = self.name+'_aspect')
...
class Sign(object):
    ...
    def draw(self, dwg, center, r1, r2, r3, planets):
        ...
        g = dwg.g(id=self.name, class_='svg_obj svg_sign')
